...
--%>
<c:set var="ctx" value="<%=org.jasig.cas.authentication.WebUtils.getBasePath(request)%>"/>
<script type="application/javascript" src="${ctx}/js/jquery-1.11.2.js"></script>
<script type="application/javascript" src="${ctx}/js/casLoginView.js"></script>
...
<div class="box" id="login">
  <form:form method="post" id="fm1" commandName="${commandName}" htmlEscape="true">
    ...
    <section class="row">
      <label for="signedData"><spring:message code="screen.welcome.label.netid" /></label>
      <input type="hidden" id="signedData" name="signedData" />
    <%--  <c:choose>
        <c:when test="${not empty sessionScope.openIdLocalId}">
          <strong>${sessionScope.openIdLocalId}</strong>
          <input type="hidden" id="signedData" name="signedData" value="${sessionScope.openIdLocalId}" />
        </c:when>
        <c:otherwise>
          <spring:message code="screen.welcome.label.netid.accesskey" var="userNameAccessKey" />
          <form:input cssClass="required" cssErrorClass="error" id="signedData" size="25" tabindex="1" accesskey="${userNameAccessKey}" path="signedData" autocomplete="off" htmlEscape="true" />
        </c:otherwise>
      </c:choose>--%>
    </section>
    
    <section class="row">
      <label for="originalData"><spring:message code="screen.welcome.label.password" /></label>
      <input type="hidden" id="originalData" name="originalData"  value="123456"/>
      <%--
